 ...
 * true
 *
 * > creative.validateRemote('https://garage.getparkave.com/some-valid-creative.json', function (err, valid) {
 *  if (!err && valid)
 *      console.log (valid)
 *  else
...
const spec = JSON.parse(fs.readFileSync(__dirname + '/../spec/creative.json', 'utf-8'))
const template = JSON.parse(fs.readFileSync(__dirname + '/../sample-creatives/minimal.json', 'utf-8'))
...
function compile (creative, inputs, opts) {
    ...
    creative.inputs.forEach(inp => {
        let key = inp.name;
        let val = typeof inputs[inp.name] !== 'undefined' ? inputs[inp.name] : inp.default;
    ...
    let html = `<style>${stylesheet}</style><div>${markup}</div><script>${script}</script>`;
    html = html.replace(`"{DATA}"`, JSON.stringify(final_inputs));
